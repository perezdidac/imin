...
// THIS METHOD RETURNS THE USER DATA OF A GIVEN USER.
...
if (isset($public_user_id))
{
	...
	if (strlen($public_user_id) == $public_user_len)
	{
		...
		if ($database_select == true)
		{
			...
			$query = "SELECT picture, user_name FROM user_data, users WHERE users.public_user_id = '$public_user_id' AND ref_user_id = user_id";
			...
			if ($query_result)
			{
				...
				if ($user_data)
				{
					...
					$result = json_encode(array('error_code'=>$error_code_success, 'user_data'=>$user_data));
				}
				else
				{
					// Query error
