...
// GIVEN AN EVENT. THE RESPONSE IS FORMATTED AS
// A JSON OBJECT.
...
if (isset($public_event_id))
{
	...
	if (strlen($public_event_id) == $public_event_len)
	{
		...
		if ($database_select == true)
		{
			...
			$query = "SELECT hash FROM pictures, events WHERE events.public_event_id = '$public_event_id' AND ref_event_id = event_id";
